...
const ProjectList: React.FC = () => {
  ...
  const getFilteredProjects = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(project => project.name.toLowerCase().includes(searchTerm.toLowerCase()) || project.client.toLowerCase().includes(searchTerm.toLowerCase()) || project.responsible.toLowerCase().includes(searchTerm.toLowerCase()) || project.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())) || project.teamMembers && project.teamMembers.toLowerCase().includes(searchTerm.toLowerCase()));
    }
  ...
  const generateEnhancedPDF = (projectsData: Project[]) => {
    ...
    projectsData.forEach((project, projectIndex) => {
      ...
      yPos += 8;
      const daysInProgress = project.startDate ? Math.floor((new Date().getTime() - new Date(project.startDate).getTime()) / (1000 * 60 * 60 * 24)) : 0;
  ...
  return <TooltipProvider>
      <Layout>
        <div className="space-y-6">
          {/* Deadline Alert */}
          {showDeadlineAlert && <AlertDialog open={showDeadlineAlert} onOpenChange={setShowDeadlineAlert}>
                ...
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription asChild>
                    <div className="space-y-4">
                      {getProjectsDueToday().length > 0 && <div>
                          <div className="flex items-center gap-2 font-medium text-orange-600 mb-2">
                            📅 Vencem Hoje ({getProjectsDueToday().length})
                          </div>
                          {getProjectsDueToday().map(project => <div key={project.id} className="bg-orange-50 p-3 rounded border">
                      ...
                      {getOverdueProjects().length > 0 && <div>
                          <div className="flex items-center gap-2 font-medium text-red-600 mb-2">
                            ⚠️ Atrasados ({getOverdueProjects().length})
                          </div>
                          {getOverdueProjects().map(project => <div key={project.id} className="bg-red-50 p-3 rounded border">
                              <div className="font-medium">{project.name}</div>
                              <div className="text-sm text-gray-600">Cliente: {project.client}</div>
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-foreground text-lg font-light">🚀 Gerencie todos os seus projetos em um só lugar</p>
            </div>
            
            <Tooltip>
              <TooltipTrigger asChild>
                <Button onClick={() => setIsCreateDialogOpen(true)} className="bg-white text-foreground border border-border hover:bg-accent">
          ...
          {/* Filter Section */}
          {activeTab === 'active' && <div className="flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input placeholder="Buscar projetos por nome, cliente, responsável, tags ou envolvidos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
              </div>
          ...
          {/* Bulk Actions */}
          {selectedProjects.length > 0 && <div className="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {activeTab === 'finished' && <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button size="sm" variant="outline">
            ...
            <TabsContent value={activeTab} className="mt-6">
              {filteredProjects.length === 0 ? <div className="text-center py-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredProjects.map(project => <div key={project.id} className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3 flex-1">
                            <Checkbox checked={selectedProjects.includes(project.id)} onCheckedChange={checked => handleSelectProject(project.id, checked as boolean)} />
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                <Link to={`/project/${project.id}`} className="font-semibold text-lg text-gray-900 hover:text-blue-600 cursor-pointer transition-colors">
                                  {project.name}
                                </Link>
                                ...
                                <div className="flex gap-1">
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button variant="ghost" size="sm" onClick={() => window.location.href = `/project/${project.id}`} className="h-6 w-6 p-0">
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button variant="ghost" size="sm" onClick={() => handleEditProject(project)} className="h-6 w-6 p-0">
                                    ...
                                    <DropdownMenuContent align="end">
                                      ...
                                      <AlertDialog>
                                        ...
                                        <AlertDialogContent>
                                          ...
                                          <AlertDialogFooter>
                                            <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                            <AlertDialogAction onClick={() => handleStatusChange(project, 'deleted')} className="bg-red-600 hover:bg-red-700">
                                              Excluir
                                            </AlertDialogAction>
                                          </AlertDialogFooter>
                                        </AlertDialogContent>
                              ...
                              {project.tags && project.tags.length > 0 && <div className="flex flex-wrap gap-1 mb-2">
                                  {project.tags.slice(0, 3).map((tag, index) => <span key={index} className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">
                                      {tag}
                                    </span>)}
                                  {project.tags.length > 3 && <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs">
                                      +{project.tags.length - 3}
                                    </span>}
                                </div>}

                              {/* Involved People */}
                              {project.teamMembers && <p className="text-xs text-gray-500 mb-2">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner">
                              <div className={`h-3 rounded-full transition-all duration-500 ${getProgressBarColor(project.progress)} shadow-sm`} style={{
      ...
      if (!open) {
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {editingProject && <ProjectForm initialData={editingProject} onSubmit={() => {
